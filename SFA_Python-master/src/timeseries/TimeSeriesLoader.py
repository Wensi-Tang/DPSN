...
uv_dir = os.getcwd()[:-5] + "/datasets/UCRArchive_2018/"
...
def uv_load(dataset_name):
    try:
        train = {}
        test = {}
        train_raw = pd.read_csv((uv_dir + dataset_name + "/" + dataset_name + "_TRAIN.tsv"), sep="\t", header=None)
        test_raw = pd.read_csv((uv_dir + dataset_name + "/" + dataset_name + "_TEST.tsv"), sep="\t", header=None)
